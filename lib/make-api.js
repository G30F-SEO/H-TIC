...
// Resolve the organizationId from the teamId
...
async function getOrganizationId() {
  ...
  if (!res.ok) {
    ...
    throw new Error(`getOrganizationId (${res.status}): ${err}`)
  }
  const data = await res.json()
  const team = data.team || data
  _cachedOrgId = team.organizationId
  console.log('[make-api] Resolved organizationId:', _cachedOrgId, 'from teamId:', teamId)
  return _cachedOrgId
}
...
export async function cloneScenario(name) {
  ...
  console.log('[make-api] cloneScenario template:', templateScenarioId, 'orgId:', organizationId)
  const res = await fetch(`${baseUrl()}/scenarios/${templateScenarioId}/clone`, {
    method: 'POST',
    headers: headers(),
    body: JSON.stringify({ name: `H-TIC — ${name}`, organizationId }),
  })
  if (!res.ok) {
    const err = await res.text()
