...
async function ensureTable() {
  ...
  // Quick check: try reading from the table
  const { error } = await sb.from('htic_store').select('key').limit(1)
  if (!error) {
    _tableReady = true
    return true
  }
  ...
  if (error.code === '42P01' || error.message?.includes('does not exist')) {
    console.error('[db] Table htic_store missing — call /api/setup to create it')
    return false
  }
  return false
}
